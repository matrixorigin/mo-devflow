...
const testingIssueQueueFilters = [
  "all",
  "stale",
  "attention",
  "unlinked",
...
function hashEnumParam<T extends string>(hash: string, key: string, allowedValues: readonly T[], fallback: T): T {
...
export function prBoardTabFromHash(hash: string): PrBoardTab {
  ...
  const testerFilter = dashboardHashParams(hash).get("tester")?.trim();
  if (isTestingPrScope(scope) || issueQueueFilter !== "all" || testerFilter) {
    return "testing";
  }
...
export function testingIssueTesterFilterFromHash(hash: string): string | null {
  const raw = dashboardHashParams(hash).get("tester")?.trim();
...
export function dashboardHashForView(view: DashboardView, options: DashboardHashOptions = {}): string {
  ...
  if (view === "PRs") {
    ...
    if (nextPrBoardTab === "testing") {
      ...
      if (options.testingIssueTesterFilter) {
        params.set("tester", options.testingIssueTesterFilter);
      }
    }
  }
  if (view === "People") {
...
export function syncHealthCursorText(cursorValue: string | null): string | null {
  ...
  try {
    ...
    const prWatermark = parsed.openPullRequestsWatermarkReached === true ? "open PRs reached previous watermark" : null;
...
function personalTokenStatus(user: NonNullable<SessionView["user"]> | null): {
  ...
  if (!hasSavedPersonalToken(user)) {
    return {
      ...
      detail:
        "The account is logged in, but GitHub write actions are disabled until this user connects a personal token."
    };
  }
  return {
...
export function prScopeHelp(filter: PrScopeFilter): string | null {
  if (filter === "no_issue") {
    return "PR detail and relationship sync completed, but no related issue is visible in cached relationship data or PR text.";
  }
  if (filter === "issue_link_pending") {
    return "Issue-link sync is still incomplete for these PRs. Do not treat them as unlinked yet.";
  }
  if (filter === "evidence_pending") {
    return "PR review, CI, merge, or issue-link cache is still incomplete for these rows.";
  }
  if (filter === "testing_evidence_gap") {
    return "A linked issue is in testing, but the linked PR cache is incomplete or not synchronized yet.";
  }
  return null;
}
...
function personCardFocus(person: PersonSummary, personal: PersonalActionView | undefined): PersonCardFocus {
  ...
  if (testingWork > 0) {
    return {
      title: "Track issue testing result",
    ...
    <div className="team-overview">
      <section className="team-command-panel">
        <div className="team-command-heading">
          <div>
            <Title level={4}>Team Board</Title>
            <Text type="secondary">
              Active s-1/s0, PR attention, issue testing, and people load. Generated {formatDate(data.sync.generatedAt)}{" "}
              | {data.repo.timezone}
            </Text>
          </div>
          <Space size={[6, 6]} wrap>
        ...
        <details className="team-evidence-disclosure">
          <summary>
            <span>Setup, sync, and secondary checks</span>
...
function TeamDataStatusStrip({
  ...
  const cacheTone = data.sync.staleObjects > 0 ? "critical" : data.sync.partialObjects > 0 ? "attention" : "good";
  const cacheValue =
...
function teamCommandActions({
  ...
  if (issuesWithoutPr.length > 0) {
    actions.push({
      ...
      detail: "confirm whether execution started, or connect the linked PR in cache",
      tone: "critical",
  ...
  return (
    <section className="team-command-board" aria-label="Team command queue">
      <div className="team-command-board-heading">
        <div>
          <Text strong>Secondary Checks</Text>
          <Text type="secondary">Additional queues after the main board.</Text>
        </div>
  ...
  return [
    {
      ...
      detail: access.githubOAuthConfigured
        ? "OAuth browser sessions can be created."
    ...
    },
    {
      ...
      detail: access.serviceReadTokenConfigured
        ? "Polling/backfill has authenticated read quota."
        : "Evidence may stay partial.",
    ...
    },
    {
      ...
      detail: access.writeBackEnabled
        ? "Confirmed writes require login and personal token."
        : "Read-only workflow fixes.",
...
function HealthLayerRow({
  ...
  return (
    <article className="health-layer-row">
      ...
      <Tooltip title={authenticated ? "Queue this sync layer" : "Sign in with GitHub to queue worker refresh jobs"}>
        <span>
...
function teamPrNextAction(pr: PendingPrView): string {
  const syntheticItem: PersonalActivityItem = {
    ...
    ownerScope: null,
    ownerReason: null,
    phase: isTestingQueuePr(pr) ? "Linked issue in testing" : "Pending PR",
...
function cacheEvidenceImpactItems(data: DashboardSummary): CacheEvidenceImpactItem[] {
  ...
  const items: CacheEvidenceImpactItem[] = [
    ...
    },
    {
      ...
      detail: `${testingEvidenceGaps} issue testing or linked PR records depend on incomplete cache evidence`,
  ...
  return (
    <div className="cache-repair-plan">
      ...
      <div className="cache-repair-reasons">
        ...
        {!authenticated ? <Text type="secondary">Manual repair queueing requires GitHub sign-in.</Text> : null}
      </div>
    </div>
  );
}
    ...
    <section className={`freshness-bar ${expanded ? "freshness-bar-expanded" : "freshness-bar-compact"}`}>
      <div className="freshness-topline">
        <Space className="freshness-main" size={[8, 8]} wrap>
          ...
          <Tag color={refreshing ? "blue" : autoRefreshError ? "orange" : "green"}>{refreshModeText}</Tag>
        </Space>
        <Button
          size="small"
          type="text"
            ...
            <div className="evidence-alert-quick-row">
              ...
              {compactRepair.layers.length > 0 ? (
                <Space size={6} wrap className="evidence-alert-repair-actions">
                  <Tooltip
                    title={authenticated ? "Queue the recommended cache repair layers" : "Sign in with GitHub first"}
                  >
                    <Button
                      size="small"
                      type="primary"
...
function PrBoardSummary({
  ...
  return (
    <div className="critical-board-summary pr-board-summary" aria-label="Pending PR summary">
      ...
      />
      <CriticalBoardStat
        label="issue testing cache pending"
...
function PeopleFocusQueue({
  ...
  return (
    <section className="people-focus-queue" aria-label="People workload focus">
...
function testingQueueNextAction(pr: PendingPrView): string {
  ...
  return "Confirm issue testing status";
}
...
function TestingCommandBoard({
  ...
  onTestingIssueTesterFilterChange,
  onOpenPrsFilter
}: {
  ...
  onTestingIssueTesterFilterChange: (tester: string | null) => void;
  onOpenPrsFilter: (scope: PrScopeFilter) => void;
}) {
  ...
  const openTesterFilter = (tester: string | null) => {
    onTestingIssueTesterFilterChange(tester);
    window.setTimeout(scrollToIssueQueue, 0);
  };
  ...
  return (
    <div className="testing-command-board">
      ...
      <TestingIssueQueuePanel
        ...
        testerFilter={testingIssueTesterFilter}
        issues={testing.issues}
        onFilterChange={onTestingIssueFilterChange}
        onTesterFilterChange={onTestingIssueTesterFilterChange}
      />
          ...
          <div className="testing-tester-strip">
            {pagedTesterRows.visibleItems.map((tester) => (
              <button
                ...
                } ${testingIssueTesterFilter === tester.login ? "testing-tester-card-active" : ""}`}
                key={tester.login}
                onClick={() => openTesterFilter(testingIssueTesterFilter === tester.login ? null : tester.login)}
              >
                <div>
        ...
        <details className="secondary-disclosure testing-pr-evidence-disclosure">
          <summary>
            ...
            <Space size={[4, 4]} wrap>
              ...
              <button
                type="button"
                className={`inline-filter-chip ${evidenceGapPrs.length > 0 ? "" : "inline-filter-chip-muted"}`}
                onClick={(event) => {
                  event.preventDefault();
                  event.stopPropagation();
              ...
              <span className="secondary-summary-note">
                PR cards show execution status only; issue signals control testing.
              </span>
            </Space>
          </summary>
          <div className="secondary-disclosure-body testing-command-lanes">
            ...
            <TestingQueueLane
              ...
              visibleLimit={6}
              tone="normal"
              emptyText="No active linked issue testing movement outside waiting lanes"
            />
          </div>
        </details>
      ) : null}
...
function testingIssueMatchesTester(issue: TestingIssueQueueView, testerFilter: string | null): boolean {
  if (!testerFilter) {
    return true;
  }
  const normalizedTester = testerFilter.toLowerCase();
  return issue.testers.some((tester) => tester.toLowerCase() === normalizedTester);
}
...
function TestingIssueQueuePanel({
  ...
  onTesterFilterChange: (tester: string | null) => void;
}) {
  ...
  const testerScopedIssues = issues.filter((issue) => testingIssueMatchesTester(issue, testerFilter));
  ...
  const sortedIssues = sortTestingIssueQueue(
    testerScopedIssues.filter((issue) => testingIssueMatchesFilter(issue, filter)),
    sort
  );
  const pagedIssues = usePagedList(
    sortedIssues,
    cardListDefaultPageSize,
    `${filter}:${testerFilter ?? "all-testers"}:${sort}:${issues.length}`
  );
        ...
        <Space size={[6, 6]} wrap>
          {testerFilter ? (
            <button
              type="button"
              className="inline-filter-chip inline-filter-chip-active"
              onClick={() => onTesterFilterChange(null)}
            >
              Tester {testerFilter} | clear
            </button>
          ) : null}
      ...
      <section className="observed-thread-panel" aria-label="Observed issue and PR flow">
        <div className="observed-thread-panel-title">
          <span>
            ...
            <Text type="secondary">Active issue first, then linked PR status</Text>
          </span>
          <Space size={[4, 4]} wrap>
...
function actionQueueFilterDescription(filter: PersonalActionQueueFilter): string {
  ...
  if (filter === "testing") {
    return "Issues currently in testing, plus linked PR status.";
  }
  if (filter === "prs") {
      ...
      <section className="team-command-panel personal-command-panel">
        ...
        <div className="team-monitor-grid personal-monitor-grid" aria-label="Personal flow monitor">
          ...
          <TeamMonitorTile
            ...
            detail={`${staleTestingWorkCount} waiting >24h | ${person.testingPrs.length} linked PRs | ${oldestPersonalTestingText([], person.testingIssues)}`}
      ...
      <div className="personal-rotation-grid">
        ...
        <section className="personal-rotation-lane personal-rotation-lane-attention">
          <div className="team-rotation-lane-heading">
            ...
            <Text type="secondary">issue testing status and linked PRs</Text>
          </div>
          <div className="team-rotation-list">
                ...
                <div className="personal-testing-subsection">
                  ...
                  {testingPrPage.visibleItems.length === 0 ? (
                    <Text type="secondary" className="personal-testing-empty">
                      No linked PRs are visible for current issue testing.
                    </Text>
                  ) : (
      ...
      <section className="personal-filtered-board personal-filtered-board-attention">
        <div className="subsection-heading">
          ...
          <Space size={[4, 4]} wrap>
            ...
            <Tag>{person.testingPrs.length} linked PRs</Tag>
          </Space>
        </div>
        <TestingIssueQueuePanel
          filter={testingIssueFilter}
          ...
          onTesterFilterChange={() => undefined}
        />
        {person.testingPrs.length > 0 || person.testingIssues.length === 0 ? (
          <div className="testing-linked-pr-evidence">
            ...
            <PullRequestCardList
              ...
              emptyText="No linked PRs in cache"
              onPreview={onPullRequestPreview}
            />
          </div>
        ) : null}
...
interface PersonalCriticalFlowEfficiencyRow {
  issueNumber: number;
  severity: string | null;
  aiEffortLabel: string;
  activeAgeHours: number | null;
  linkedPrs: number;
  firstPrAfterActiveHours: number | null;
  testingAfterActiveHours: number | null;
  cachePending: boolean;
}

function latestTrendMetricPoint(points: TrendMetricPoint[]): TrendMetricPoint | null {
  if (points.length === 0) {
    return null;
  }
  return [...points].sort(
...
function personalCriticalFlowEfficiencyRow(issue: CriticalIssueView): PersonalCriticalFlowEfficiencyRow {
  const activeAgeHours = issue.criticalAgeHours;
  const prLeadTimes =
    activeAgeHours === null ? [] : issue.linkedPullRequests.map((pr) => Math.max(0, activeAgeHours - pr.ageHours));
  const testingLeadTimes =
    activeAgeHours === null
      ? []
  ...
  return {
    ...
    activeAgeHours,
    linkedPrs: issue.linkedPullRequests.length,
    firstPrAfterActiveHours: prLeadTimes.length > 0 ? Math.min(...prLeadTimes) : null,
      ...
      <div className="subsection-heading">
        ...
        <Space size={[4, 4]} wrap>
          <button type="button" className="inline-filter-chip" onClick={() => onDrilldownChange("pending_pr")}>
            {person.pendingPrs.length} pending
          </button>
          <button
            type="button"
            className={`inline-filter-chip ${person.attentionPrs.length > 0 ? "" : "inline-filter-chip-muted"}`}
            onClick={() => onDrilldownChange("pr_attention")}
          >
            {person.attentionPrs.length} attention
          </button>
        </Space>
      </div>
            ...
            <div className="personal-flow-efficiency-rows">
              {visibleFlowRows.map((row) => (
                ...
                >
                  ...
                  <span>
                    <strong>{optionalHours(row.activeAgeHours)}</strong>
                    <small>s-1/s0 age</small>
                  </span>
                  <span>
                    <strong>{row.linkedPrs}</strong>
                    <small>linked PRs</small>
                  </span>
                  <span>
      ...
      <details className="secondary-disclosure personal-detail-disclosure">
        ...
        <div className="secondary-disclosure-body">
          ...
          <div className="work-lane-grid">
            ...
            <WorkLane title="Linked PRs" count={person.testingPrs.length} tone="normal">
              <PullRequestCardList
...
export default function App() {
  ...
  const [testingIssueTesterFilter, setTestingIssueTesterFilter] = useState<string | null>(() =>
    testingIssueTesterFilterFromHash(initialHash())
  );
  ...
  function openPrsWithFilter(scope: PrScopeFilter) {
    ...
    const nextTestingIssueTesterFilter = nextTab === "testing" ? testingIssueTesterFilter : null;
    setPrScopeFilter(scope);
    setPrBoardTab(nextTab);
    if (nextTab === "testing") {
      setTestingIssueQueueFilter(nextTestingIssueFilter);
      setTestingIssueTesterFilter(nextTestingIssueTesterFilter);
    } else {
      setTestingIssueTesterFilter(null);
    }
    selectView("PRs", selectedPerson, {
      prScopeFilter: scope,
      prBoardTab: nextTab,
      testingIssueQueueFilter: nextTestingIssueFilter,
      testingIssueTesterFilter: nextTestingIssueTesterFilter
    });
  }

  function openIssueTestingQueue(filter: TestingIssueQueueFilter = "all", testerFilter: string | null = null) {
    const nextScope: PrScopeFilter = "testing";
    setTestingIssueQueueFilter(filter);
    setTestingIssueTesterFilter(testerFilter);
    ...
    selectView("PRs", selectedPerson, {
      ...
      testingIssueTesterFilter: testerFilter
    });
  ...
  function switchPrBoardTab(nextTab: PrBoardTab) {
    ...
    if (view === "PRs") {
      replaceDashboardHash("PRs", selectedPerson, {
        ...
        prScopeFilter: nextScope,
        testingIssueQueueFilter,
        testingIssueTesterFilter: nextTab === "testing" ? testingIssueTesterFilter : null
      });
    }
  }
  ...
  function openDashboardViewLimit(limit: DashboardViewLimit) {
    ...
    if (target.view === "PRs") {
      ...
      const nextTestingIssueTesterFilter =
        nextTab === "testing" ? (options.testingIssueTesterFilter ?? testingIssueTesterFilter) : null;
      setPrScopeFilter(nextScope);
      setPrSort(nextSort);
      setPrBoardTab(nextTab);
      if (nextTab === "testing") {
    ...
    () => [
      ...
      },
      {
        ...
        render: (_, delivery) => {
          ...
          return (
            <Tooltip title={session?.authenticated ? "Acknowledge notification" : "Sign in with GitHub to acknowledge"}>
              <span>
                <Button
                  size="small"
  ...
  const openTestingIssueTesterFilter = (nextTesterFilter: string | null) => {
    const nextScope: PrScopeFilter = "testing";
    const nextFilter: TestingIssueQueueFilter = "all";
    setTestingIssueQueueFilter(nextFilter);
    setTestingIssueTesterFilter(nextTesterFilter);
    setPrScopeFilter(nextScope);
    setPrBoardTab("testing");
    if (view === "PRs") {
      replaceDashboardHash("PRs", selectedPerson, {
        ...
        prBoardTab: "testing",
        testingIssueQueueFilter: nextFilter,
        testingIssueTesterFilter: nextTesterFilter
      });
    }
    window.setTimeout(scrollTestingIssueQueueIntoView, 0);
  };
    ...
    <Layout className="app-shell">
      <Header className="topbar">
        ...
        <Space className="topbar-actions" size={[8, 8]} wrap>
          ...
          <Tooltip
            title={session?.authenticated ? "Queue worker refresh" : "Sign in with GitHub to queue worker refresh"}
          >
            <Button
                ...
                {prBoardTab === "testing" ? (
                  <>
                    ...
                    <TestingCommandBoard
                      ...
                      testingIssueFilter={testingIssueQueueFilter}
                      testingIssueTesterFilter={testingIssueTesterFilter}
                      onTestingIssueFilterChange={openTestingIssueQueueFilter}
                      onTestingIssueTesterFilterChange={openTestingIssueTesterFilter}
                      onOpenPrsFilter={openPrsWithFilter}
                    />
                    <div className="testing-pr-table-panel" id="testing-pr-table-panel" aria-label="Linked PR table">
                      <div className="subsection-heading subsection-heading-compact">
                      ...
                      <Table
                        ...
                        locale={{ emptyText: <Empty description="No linked PRs in cache" /> }}
                      />
                    </div>
                    <details
                      ...
                      <div className="secondary-disclosure-body">
                        {testingHasIssueTransitions ? (
                          <Table
                            ...
                            locale={{ emptyText: <Empty description="No issue testing assignment in cache yet" /> }}
                          />
                        ) : null}
                      </div>
                    </details>
              ...
              <section className="workbench-section">
                <div className="section-heading">
                  ...
                  <Space size={[6, 6]} wrap>
                    ...
                    <Tag
                      color={
                        peopleScopeFilter === "critical" ? "red" : peopleScopeFilter === "all" ? "default" : "orange"
                      }
                    >
